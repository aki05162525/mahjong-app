...
function makeReq() {
  return new NextRequest("http://localhost/api/players/p1", { method: "DELETE" });
}
...
describe("DELETE /api/players/[id]", () => {
  it("401: 未ログインは削除できない", async () => {
    ...
    const res = await DELETE(makeReq(), { params: makeParams("p1") });
    expect(res.status).toBe(401);
  });

  it("404: 存在しないプレイヤーは削除できない", async () => {
  ...
  it("409: 対局結果に記録されているプレイヤーは削除できない", async () => {
    mockFrom
      .mockReturnValueOnce(makeChain({ data: { tournament_id: "t1" }, error: null }))
    ...
    expect((await res.json()).error).toMatch(/対局結果/);
  });

  it("200: 対局結果のないプレイヤーは削除できる", async () => {
    mockFrom
      .mockReturnValueOnce(makeChain({ data: { tournament_id: "t1" }, error: null }))
      .mockReturnValueOnce(makeChain({ data: { owner_id: "owner-1" }, error: null }))
